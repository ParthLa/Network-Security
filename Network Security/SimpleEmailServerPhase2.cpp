...
//Extracting Username and Password
		...
		if(username!=message_username){
			cout<< "user-name: Folder Read Fail\n";
...
//Receive message
	...
	if (pathdir == NULL){
	 cerr<<"username folder not readable\n";
	 exit(4);
	}
	int N =0;
	while (pt = readdir(pathdir)){
	 if (pt->d_name[0]!='.'){
	  N++;
	 }
	}
	...
	send_message(in_socket,list_message
	 if(send_message(in_socket,list_message) < 0){
			cout<<"Message not sent"<<endl;
	}
	}
...
//Validates Arguments
	if (argc != 4){
		cerr<<"Invalid number of arguments\nUsage: SimpleEmailServerPhase1 <PortNum> <PasswordFile> <user-database> \n";
		exit(1);
	}
	char **x;
