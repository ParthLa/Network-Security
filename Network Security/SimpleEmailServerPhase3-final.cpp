...
int send_list(string username, const char* database, int sockfd)
{
    //cout<<username<<endl;
    DIR* p;
    struct dirent* dp;
...
int retrvm(string user, const char* database, string buf, int sockfd)
{
...
int parse_message(const char* filename, string buf, const char* database, string& user, int sockfd){
	regex quit("quit");
	regex message("(User:\\s)(.*)( Pass:\\s)(.*)");
	regex words("[^:\\s]");
	regex lists("LIST");
...
//		}
	...
	{
	cout<<"a"<<endl;
        if (send_list(user, database, sockfd)<0)
        {
            return -3;
        }else{return 1;}
	}
	...
	{
        cout<<"b"<<endl;
        if(retrvm(user, database, buf, sockfd)<0)
        {
            return -4;
        }
        else{return 1;}
...
//Receive message
	...
	int p = parse_message(filename, buf, database, username, in_socket);
	if(p<0)
	{
        close(in_socket);
        break;
